...
def crop_and_resize_mask(full_mask, box, target_size=(518, 518)):
    ...
    resized_mask = cv2.resize(cropped_mask.astype(np.float32), target_size, interpolation=cv2.INTER_AREA)
...
def save_batch_results(
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        list(
            tqdm(
                executor.map(lambda p: crop_masked_region(*p), crop_tasks),
                total=len(crop_tasks),
                desc="Saving crops",
                leave=False,
            )
        )
        list(
            tqdm(
                executor.map(lambda p: overlay_mask_on_image(*p), overlay_tasks),
...
def segment_images(path_to_images=None):
    ...
    for i in tqdm(range(0, len(data_to_process_df), BATCH_SIZE), total=total_batches, desc="Overall Progress"):
        ...
        if batch_results:
            ...
            save_batch_results(batch_results, resized_masks_list=all_resized_masks, dino_input_size=DINOV2_INPUT_SIZE)
            saving_time = time.time() - saving_start
            tqdm.write(
                f"Batch {i // BATCH_SIZE + 1}/{total_batches} | Inference: {inference_time:.2f}s, Saving: {saving_time:.2f}s"
            )
